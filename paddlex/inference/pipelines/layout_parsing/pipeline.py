...
class LayoutParsingPipeline(BasePipeline):
    ...
    def get_layout_parsing_res(
        ...
        for object_box_idx, box_info in enumerate(layout_det_res["boxes"]):
            ...
            if label == "formula":
                if len(formula_res_list) > 0:
                    ...
                    single_box_res["block_text"] = formula_res_list[formula_index][
                        "rec_formula"
                    ]
                    formula_index += 1
            elif label == "table":
                if len(table_res_list) > 0:
                    ...
                    single_box_res["block_text"] = table_res_list[table_index][
                        "pred_html"
                    ]
                    table_index += 1
            elif label == "seal":
                if len(seal_res_list) > 0:
                    ...
                    single_box_res["block_text"] = ", ".join(
        ...
        for layout_box_ids in matched_ocr_dict.values():
            ...
            if len(layout_box_ids) > 1:
                for idx in layout_box_ids:
                    ...
                    layout_parsing_res[idx]["block_text"] = "\n".join(
        ...
        for ocr_rec_box, ocr_rec_text in zip(
            ...
            single_box_res["block_text"] = ocr_rec_text
            layout_parsing_res.append(single_box_res)
